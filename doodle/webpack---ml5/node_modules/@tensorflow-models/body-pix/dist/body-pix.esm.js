...
function resizeAndPadTo(e, t, r) {
    ...
    return {
        resizedAndPadded: tidy(function() {
            var t;
            return t = r ? e.reverse(1).resizeBilinear([i, o]) : e.resizeBilinear([i, o]), pad3d(t, [
                [u, l],
                [s, d],
                [0, 0]
            ])
...
function scaleAndCropToInputTensorShape(e, t, r, a) {
    var n = t[0],
        ...
        h = c[1];
    return tidy(function() {
        return removePaddingAndResizeBack(e.resizeBilinear([i, s], !0), [n, o], [
            [u, l],
            [p, h]
        ])
    })
...
function cpuBlur(e, t, r) {
    for (var a = e.getContext("2d"), n = 0, o = 1 / (2 * Math.PI * 5 * 5), i = r < 3 ? 1 : 2, s = -r; s <= r; s += i)
        for (var d = -r; d <= r; d += i) {
            n += o * Math.exp(-(d * d + s * s) / 50)
        }
    for (s = -r; s <= r; s += i)
        for (d = -r; d <= r; d += i) a.globalAlpha = o * Math.exp(-(d * d + s * s) / 50) / n * r, a.drawImage(t, d, s);
    a.globalAlpha = 1
}
...
function toColoredPartImageData(e, t) {
    for (var r = e.width, a = e.height, n = e.data, o = new Uint8ClampedArray(r * a * 4), i = 0; i < a * r; ++i) {
        var s = Math.round(n[i]),
            d = 4 * i;
        if (-1 === s) o[d + 0] = 255, o[d + 1] = 255, o[d + 2] = 255, o[d + 3] = 255;
        else {
            var u = t[s];
